...
class ExportDialog(QDialog):
    ...
    def __init__(self, engine, parent=None):
        ...
        self._format_combo.addItem("PDF (.pdf)", "pdf")
    ...
    def _on_export(self):
        ...
        for tree_name in selected_trees:
            nodes = self._engine.get_nodes_by_tree(tree_name)
            if selected_nodes.get(tree_name):
                allowed = set(selected_nodes[tree_name])
                nodes = [n for n in nodes if n.id in allowed]
            for node in nodes:
                if not (node.label or "").strip() or not (node.description or "").strip():
                    invalid_nodes.append(f"{TREE_NAMES.get(tree_name, tree_name)} → {node.label or '(без названия)'}")
        if invalid_nodes:
            ...
            QMessageBox.warning(
                self,
                "Экспорт",
                "Перед экспортом заполните обязательные поля узлов:\n"
                "название и описание.\n\n"
                f"Проблемные узлы:\n{preview}{suffix}",
            )
            return
        ...
        if fmt == "pdf":
            self._export_pdf(path, md)
        else:
            self._export_text(path, md)

    def _export_text(self, path: str, content: str):
        try:
    ...
    def _export_pdf(self, path: str, content: str):
        """Export as PDF using reportlab if available, else fallback."""
        try:
            ...
            QMessageBox.warning(
                self, "Ошибка",
                "PDF-экспорт требует библиотеку reportlab.\n"
                "Установите: pip install reportlab\n\n"
                "Файл сохранён как Markdown.",
            )
            self._export_text(path.replace(".pdf", ".md"), content)
        except Exception as e:
            QMessageBox.warning(self, "Ошибка", f"PDF экспорт не удался:\n{e}")

    def _export_png(self, path: str):
        """Render the canvas scene as PNG."""
        from PySide6.QtGui import QImage, QPainter, QColor
        from src.utils.constants import BASE, LIGHT_BASE

        parent = self.parent()
        if parent and hasattr(parent, "_scene"):
            scene = parent._scene
            rect = scene.sceneRect()
            w, h = int(rect.width()), int(rect.height())
            if w <= 0 or h <= 0:
                QMessageBox.warning(self, "Ошибка", "Сцена пуста.")
                return
            image = QImage(w, h, QImage.Format_ARGB32_Premultiplied)
            is_dark = getattr(parent, "_is_dark", True)
            bg = BASE if is_dark else LIGHT_BASE
            ...
            painter.end()
            image.save(path)
            QMessageBox.information(self, "Готово", f"PNG сохранён:\n{path}")
            self.accept()
        else:
            QMessageBox.warning(self, "Ошибка", "Рендер PNG доступен только из основного окна.")
