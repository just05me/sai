...
ROUTE_STEP = 12
BOX_ROUTE_MARGIN = 10
EDGE_ROUTE_MARGIN = EDGE_HITBOX + 5
...
class EdgePath(QGraphicsPathItem):
    ...
    def _collect_edge_obstacles(self) -> list[QRectF]:
        ...
        for item in scene.items():
            ...
            # Edges sharing endpoint with current one may leave the same node area.
    ...
    def update_path(self):
        ...
        if self._control_points:
            pts = [p1] + [QPointF(cp["x"], cp["y"]) for cp in self._control_points] + [p2]
        else:
            o_s = _side_vector(src_side)
            o_t = _side_vector(tgt_side)
            ...
            if middle is None:
                # Degrade softly: still strictly avoid boxes.
                middle = _a_star(stub_s, stub_t, box_blocked, ROUTE_STEP)
            if middle and len(middle) >= 2:
                pts = [p1] + middle + [p2]
                pts = _compress_collinear(pts)
            else:
                # Last resort: keep previous safe path or build a global detour.
                if self._last_safe_points:
                ...
                # Hard safety guard: never accept route crossing box hitboxes.
                if _polyline_hits_rects(pts, box_blocked):
                    return
